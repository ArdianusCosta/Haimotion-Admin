...
export function UangMukaPembelianPage() {
  const { t } = useLanguage()
  return (
    <div className="flex flex-col gap-6 pb-8 text-foreground">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-border pb-4 mb-6 gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight mb-1">{t('Purchase Down Payment')}</h1>
          <p className="text-sm text-muted-foreground">{t('Manage data for')} {t('Purchase Down Payment')}</p>
        </div>
